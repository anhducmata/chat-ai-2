...
function IconLogo({ className, ...props }: React.ComponentProps<'svg'>) {
  // Default to a neutral color for server rendering
  const [fill, setFill] = useState('#666666')
  const { resolvedTheme } = useTheme()
  
  // Only update the fill color on the client side
  useEffect(() => {
    const isDark = resolvedTheme === 'dark'
    setFill(isDark ? "#ffffff" : "#000000")
  }, [resolvedTheme])
